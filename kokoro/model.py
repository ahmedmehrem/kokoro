...
class KModel(torch.nn.Module):
    ...
    def forward_with_tokens(
        ...
        text_mask = (
            torch.arange(input_lengths.max().item())
            .unsqueeze(0)
        ...
        duration = torch.sigmoid(duration).sum(-1) / speed
        ...
        pred_dur = cast(torch.LongTensor, pred_dur)
        pred_aln_trg = cast(torch.FloatTensor, pred_aln_trg)
        return (
            audio,
            pred_dur,
            pred_aln_trg,
    ...
    def forward(
        ...
        ref_s = cast(torch.FloatTensor, ref_s.to(self.device))
        audio, pred_dur, align_matrix = self.forward_with_tokens(
        ...
        return (
            self.Output(audio=audio, align_matrix=align_matrix, pred_dur=pred_dur)
            if return_output
            else audio
        )
...
class KModelForONNX(torch.nn.Module):
    ...
    def forward(
        ...
        waveform, duration, align_matrix = self.kmodel.forward_with_tokens(
            input_ids, ref_s, speed
        )
        return waveform, duration, align_matrix
